...
uniform float _cameraMoveVal;

uniform float _isWhiteOut;
uniform float _whiteOutVal;
uniform float _isBlackOut;
uniform float _blackOutVal;
...
float speed=-.25;
  float tval=-0.;
  //r*ro.x,-1.,r*ro.z
  if(_cameraPositionIndex==3){
    d.d3=d3(p+vec3(0.,-2.,-15.0+_cameraMoveVal*18.0));
  }else if(_cameraPositionIndex==4){
    d.d3=d3(p+vec3(0.,-2.,2.0+_cameraMoveVal*104.0));
    //ro=vec3(0.,-2.,2.0+_cameraMoveVal*104.0);
  }else{
...
float speed=-.25;
 ...
 if(_cameraPositionIndex==0){
    ta=vec3(0.);
    ro=vec3(cos(time*speed)*radius,0.0,sin(time*speed)*radius);
 }else if(_cameraPositionIndex==1){
    ta=vec3(cos(time*speed)*radius*0.95,0.0,sin(time*speed)*radius*0.95);
    ro=vec3(cos(time*speed+0.1)*radius,-0.5,sin(time*speed+0.1)*radius);
 }else if(_cameraPositionIndex==2){
    ta=vec3(cos(time*speed)*radius*0.5,0.0,sin(time*speed)*radius*0.5);
    ro=vec3(cos(time*speed-0.5)*radius*0.75,6.0,sin(time*speed-0.5)*radius*0.75);
 }else if(_cameraPositionIndex==3){
    ta=vec3(0.);
    ro=vec3(0.,2.,26.0-_cameraMoveVal*24.0*1.5);
 }else if(_cameraPositionIndex==4){
     ta=vec3(0.);
    //ta=vec3(0., -0.1*pow(_cameraMoveVal+0.1,2.0)+0.65,2.0+_cameraMoveVal*27.0);
    //ro=vec3(0.,2.,0.0-_cameraMoveVal*24.0*1.5-2.0);
    ro=vec3(0.,2.,4.0+_cameraMoveVal*104.0);
 }
 ...
 vec3 rd=normalize(vec3(st.x*cSide+st.y*cUp+cDir*depth));
  ...
  if(d.d1<0.001){
    col+=vec3(exp(-.15*t))*vec3(0.45,0.25,0.16)*acc*0.05;
  }
  ...
  if(_cameraPositionIndex==3){
  float startVal=0.5;
    col+=(_cameraMoveVal>startVal)? vec3(1.0)*(1.0-(1.0-min(_cameraMoveVal,1.0))/(1.0-startVal)):vec3(0.0);
  }
